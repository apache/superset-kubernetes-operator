...
- **Lifecycle tasks** — parent-owned Jobs + ConfigMap. Forward cascade of sequential tasks: "seed" (seed the metastore from an external source database), "backup" (logical dump of the metastore before an upgrade), "migrate" (`superset db upgrade`), "rotate" (`superset re-encrypt-secrets` for secret key rotation), and "init" (`superset init`). A separate, out-of-band "restore" task replays a captured backup into the metastore under an approval gate. Each task can be independently disabled via `disabled: true`. Seed supports `cronSchedule` for periodic re-execution.
...
- **Presence = enabled.** No `enabled: true/false`. If `celeryWorker: {}` is set, workers deploy. Lifecycle tasks (migrate, init) run by default; disable individual tasks via `disabled: true`. Seed runs when `spec.lifecycle.seed` is set. Backup runs when `spec.lifecycle.backup` is set (and a prior image exists — it is a pre-upgrade capture). Rotate runs when `spec.lifecycle.rotate` is set. Restore runs when `spec.lifecycle.restore` is set, but only after explicit approval.
...
- **Lifecycle tasks**: `spec.lifecycle` on the parent CRD (type `LifecycleSpec`) defines a forward cascade of sequential tasks: "seed" (seed the metastore from an external source database), "backup" (logical dump of the metastore before an upgrade), "migrate" (`superset db upgrade`), "rotate" (`superset re-encrypt-secrets` for secret key rotation), and "init" (`superset init`). The parent controller sequences them (each waits for the previous), gates component deployment, and triggers re-runs by deleting and recreating deterministic task Jobs when checksums change. Tasks run as Jobs with `backoffLimit: 0`; operator-managed retry/backoff and durable state are stored in parent `status.lifecycle`. Each task can be independently disabled via `disabled: true`. Seed supports `cronSchedule` for periodic re-execution. Checksums cascade: a re-seed triggers re-backup, re-migrate, re-rotate, re-init. Version comparison detects upgrade vs downgrade; downgrades are blocked (phase: `Blocked`). `upgradeMode: Automatic` (default) runs tasks immediately; `Supervised` waits for an annotation approval before proceeding (phase: `AwaitingApproval`). The "backup" task (a `seed`/`backup`/`restore` task runs in a database-tool image — postgres/mysql client — not the Superset image) writes a logical dump to a pluggable destination (`Volume` PVC or `ObjectStore` S3) and records the artifact in `status.lifecycle.backups`; it runs pre-upgrade only (skipped on first install). A separate, out-of-band "restore" task (`reconcileRestore`, runs before the upgrade gate) replays a selected backup into the metastore under its own approval gate — annotation `superset.apache.org/approve-restore` matching a deterministic token (`restoreApprovalToken`); on completion it advances `LastLifecycleImage`, which dissolves the downgrade-blocked state for that transition, and (unless `skipPreRestoreSnapshot`) takes a safety snapshot of the current DB first. Lifecycle gates component deployment — components are not updated until all enabled tasks complete. Dev-mode-only `adminUser` and `loadExamples` fields append steps to the init task command. Parent status tracks `LastLifecycleImage` and `Lifecycle *LifecycleStatus` (with lifecycle `Phase` enum: `Seeding`, `BackingUp`, `Draining`, `Migrating`, `Rotating`, `Initializing`, `Restoring`, `Complete`, etc.). Drain only runs when a task that will run requires drain and at least one configured component has desired replicas greater than zero; when it runs, parent brings up maintenance Deployment only if `maintenancePage` is set and an existing web-server workload is present, switches the parent-owned web-server Service selector to maintenance-page labels, drains components, runs tasks, waits for web-server ready, switches selector back, and deletes maintenance resources.
...
| `lifecycle` (restore) | `restore` | `superset` |
