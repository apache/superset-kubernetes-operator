...
- **Per-component config rendering.** All Python components get `SECRET_KEY` rendered from `SUPERSET_OPERATOR__SECRET_KEY`. Web gets port config. Structured metastore renders an f-string URI from `SUPERSET_OPERATOR__DB_*` env vars. When `spec.valkey` is set, operator renders all cache configs (`CACHE_CONFIG`, `DATA_CACHE_CONFIG`, etc.), `CeleryConfig`, and `RESULTS_BACKEND` from `SUPERSET_OPERATOR__VALKEY_*` env vars. Websocket gets no `superset_config.py` (Node.js); when `spec.realtime.webSocket` is set the operator injects its JWT secret and coordination Redis connection as env vars.
- **Realtime (`spec.realtime`).** Centralizes the cross-component wiring for Global Async Queries and the realtime websocket transport. `asyncQueries` sets the `GLOBAL_ASYNC_QUERIES` feature flag and adds the `superset.tasks.async_queries` Celery import plus the `reap_orphaned_tasks` beat schedule; `webSocket` sets `WEBSOCKET_ENABLE`/`WEBSOCKET_URL` on Python components and shares a JWT secret (`jwtSecret`/`jwtSecretFrom`) with the websocket workload. Both require `spec.valkey.distributedCoordination`. The websocket workload stays a top-level `spec.websocketServer` component and now ships in the official Superset image (launched via `/app/docker/entrypoints/run-websocket.sh`).
- **`spec.baseUrl`.** External browser-visible base URL. When set, the operator derives the websocket URL, the websocket `ALLOWED_ORIGINS` allowlist (defaults to that single origin; overridable via `realtime.webSocket.allowedOrigins`), and `WEBDRIVER_BASEURL_USER_FRIENDLY` (Alerts & Reports hyperlinks). Falls back to `spec.networking` for URL/origin derivation when `baseUrl` is unset. The internal `WEBDRIVER_BASEURL` (headless-browser render target) is always operator-managed — it points at the web-server Service and is not user-configurable.
