...
- **Superset** (parent) — top-level CR with shared spec (top-level + per-component), environment, secretKey/secretKeyFrom, metastore (literal or Secret-backed connection fields), valkey (cache/broker/results with literal or Secret-backed connection fields), config, LifecycleSpec, NetworkingSpec, MonitoringSpec
- **Lifecycle tasks** — parent-owned Jobs + ConfigMap. Four sequential tasks: "seed" (database snapshot from external source), "migrate" (`superset db upgrade`), "rotate" (`superset re-encrypt-secrets` for secret key rotation), and "init" (`superset init`). Each task can be independently disabled via `disabled: true`. Seed supports `cronSchedule` for periodic re-execution. An optional out-of-cascade "backup" task dumps and verifies the metastore before migrate/rotate.
...
- **Presence = enabled.** No `enabled: true/false`. If `celeryWorker: {}` is set, workers deploy. Lifecycle tasks (migrate, init) run by default; disable individual tasks via `disabled: true`. Seed runs when `spec.lifecycle.seed` is set. Rotate runs when `spec.lifecycle.rotate` is set. Backup runs when `spec.lifecycle.backup` is set.
- **Secrets never touch ConfigMaps.** In prod mode, CRD CEL validation rejects inline `secretKey`, `metastore.uri`, `metastore.password`, and `valkey.password`. Use the corresponding `*From` fields to reference Kubernetes Secrets (operator injects `valueFrom.secretKeyRef` env vars). Structured metastore, Valkey, and seed-source connection fields also have `*From` counterparts for provisioner-owned connection details. In dev mode, inline secrets are allowed.
...
- `api/v1alpha1/` — CRD type definitions
  ...
  - `superset_types.go` — SupersetSpec (environment, secretKey/secretKeyFrom, metastore, valkey, config, sqlaEngineOptions, autoscaling, podDisruptionBudget), component specs (GunicornSpec on webServer, CeleryWorkerProcessSpec on celeryWorker, SQLAlchemyEngineOptionsSpec on all Python components except Flower), LifecycleSpec (seed/backup/migrate/rotate/init tasks, upgradeMode, maintenancePage), AdminUserSpec, NetworkingSpec, MonitoringSpec, status types (LifecycleStatus, ComponentStatusMap, LastLifecycleImage)
...
- `internal/controller/` — Reconciler implementations
  ...
  - `lifecycle_backup.go` — Backup task: settled-baseline checksum, guard placement, dump/verify/manifest/retention scripts, Job spec, termination-message result parsing into `status.lifecycle.backups`
...
- **Environment modes**: `environment: Development` allows inline `secretKey`, `metastore.uri`, `metastore.password`, `valkey.password`, `lifecycle.adminUser`, and `lifecycle.loadExamples`. `environment: Production` (default) rejects these via CRD validation; use the corresponding `*From` fields to reference Kubernetes Secrets (operator injects `valueFrom.secretKeyRef` env vars).
...
- **External connection values**: Every typed connection field that a Kubernetes provisioner may publish after reconciliation (host, port, database, username, password) must support both a literal and a mutually exclusive `*From` `SecretKeySelector`. Use per-field selectors because provisioner key names and ownership boundaries vary. Pass selectors through as `valueFrom.secretKeyRef`; never read their values into the operator or place them in ConfigMaps, status, checksums, or logs. Apply the pattern to lifecycle-only connection tuples as well as long-running components.
- **Defaults with literal/`*From` pairs**: Kubernetes applies CRD/OpenAPI defaults before CEL validation. Do not add a kubebuilder default to the literal member of a mutually exclusive literal/`*From` pair; selector-only input would be defaulted into an invalid two-field configuration. Apply the effective default in resolution/env construction when both forms are absent, retain typed validation for explicit literals, document the runtime default, and cover selector-only admission with envtest.
...
- **Lifecycle tasks**: `spec.lifecycle` on the parent CRD (type `LifecycleSpec`) defines up to four sequential tasks: "seed" (database snapshot from external source), "migrate" (`superset db upgrade`), "rotate" (`superset re-encrypt-secrets` for secret key rotation), and "init" (`superset init`). The parent controller sequences them (migrate waits for seed, rotate waits for migrate, init waits for rotate), gates component deployment, and triggers re-runs by deleting and recreating deterministic task Jobs when checksums change. Tasks run as Jobs with `backoffLimit: 0`; operator-managed retry/backoff and durable state are stored in parent `status.lifecycle`. Each task can be independently disabled via `disabled: true`. Seed supports `cronSchedule` for periodic re-execution. Checksums cascade: a re-seed triggers re-migrate, which triggers re-rotate, which triggers re-init. Any change to the lifecycle image tag re-runs migrate (`superset db upgrade`) regardless of direction — downgrades are not blocked (Superset's `db downgrade` is poorly tested and often breaks, so the operator never runs down migrations and relies on the optional pre-upgrade backup task, `spec.lifecycle.backup`, as the safety net). Backup is out of the checksum cascade: it runs once per lifecycle run before the first not-yet-started pending migrate/rotate, keyed on `status.lifecycle.settledChecksum` (advanced only when a run fully completes); it is mutually exclusive with seed. It runs before drain by default (`requiresDrain` defaults to false), so a failed backup blocks the upgrade while the current version keeps serving. The default scripts verify the dump in full before committing it, re-hash it after commit, write a JSON manifest (commit marker), and optionally prune by `retention.keepLast` (manifests matched by UID); the Job reports its result via the container termination message, which the controller parses strictly into `status.lifecycle.backups` and the task message. `upgradeMode: Automatic` (default) runs tasks immediately; `Supervised` waits for an annotation approval before proceeding (phase: `AwaitingApproval`). Lifecycle gates component deployment — components are not updated until all enabled tasks complete. Dev-mode-only `adminUser` and `loadExamples` fields append steps to the init task command. Parent status tracks `LastLifecycleImage` and `Lifecycle *LifecycleStatus` (with lifecycle `Phase` enum: `Seeding`, `Draining`, `BackingUp`, `Migrating`, `Rotating`, `Initializing`, `Restoring`, `Complete`, etc.). Drain only runs when a task that will run requires drain and at least one configured component has desired replicas greater than zero; when it runs, parent brings up maintenance Deployment only if `maintenancePage` is set and an existing web-server workload is present, switches the parent-owned web-server Service selector to maintenance-page labels, drains components, runs tasks, waits for web-server ready, switches selector back, and deletes maintenance resources.
...
| `lifecycle` (backup) | `backup` | `superset` |
