...
| Command | Description |
|---|---|
| `make codegen` | Regenerate all generated artifacts (CRDs, DeepCopy, Helm CRDs, chart README, API docs, supported-versions table, make-commands tables). |
...
`charts/superset-operator/README.md` is generated by [`helm-docs`](https://github.com/norwoodj/helm-docs) from `# --` annotations in `values.yaml`. After modifying `values.yaml` or `values.schema.json`, run `make codegen` (or `make helm-docs`) to regenerate it. CI verifies nothing is stale.

The overall structure of the README is controlled by `charts/superset-operator/README.md.gotmpl`. Fields that accept Kubernetes types carry a `description` in `values.schema.json` that links to [pkg.go.dev](https://pkg.go.dev), making both helm-docs output and ArtifactHub display self-explanatory without inlining the full K8s schemas.
