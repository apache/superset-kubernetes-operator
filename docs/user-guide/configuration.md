...
| `SUPERSET_OPERATOR__BACKUP_DIR`, `SUPERSET_OPERATOR__BACKUP_FROM_TAG`, `SUPERSET_OPERATOR__BACKUP_FIRST_RUN`, `SUPERSET_OPERATOR__BACKUP_UID`, `SUPERSET_OPERATOR__BACKUP_METADATA`, `SUPERSET_OPERATOR__BACKUP_KEEP_LAST` | Operator-internal | Operator (backup task Job only, from `spec.lifecycle.backup`) | Backup mount path (`/backup`, only with a destination), sanitized image tag the database was on before the run (`initial` on the first run), whether no lifecycle run has completed yet, the Superset UID (retention matches manifests by it), the JSON metadata embedded in each manifest (name, namespace, UID, database type, from/to images), and `retention.keepLast` (only when set) |
