...
| — (chart has no equivalent) | `spec.lifecycle.backup` | **Operator-only.** Dumps the metastore (`pg_dump`/`mysqldump`) to an existing PVC, or via a custom command, before migrate or rotate runs, so an upgrade can be reverted by restoring the dump. Runs once per lifecycle run after drain; mutually exclusive with `seed`. |
