...
| — (chart has no equivalent) | `spec.lifecycle.backup` | **Operator-only.** Dumps the metastore (`pg_dump`/`mysqldump`) to an existing PVC, or via a custom command, before migrate or rotate runs, so an upgrade can be reverted by restoring the dump. Runs once per lifecycle run, before drain by default; each dump is verified and recorded with a manifest and in `status.lifecycle.backups`; optional `retention.keepLast`. Mutually exclusive with `seed`. |
