...
- **Pre-upgrade metastore backup.** A new optional `spec.lifecycle.backup` task dumps the metastore (`pg_dump -Fc` or `mysqldump`) to an existing PersistentVolumeClaim, or through a custom command, immediately before a migrate or rotate run, so a failed or unwanted upgrade can be reverted by restoring the dump. It runs before drain by default, so a slow backup adds no downtime and a failed backup blocks the upgrade while the current version keeps serving; set `requiresDrain: true` to drain first. Each dump is read back in full before it is committed, re-hashed after commit, and recorded in a JSON manifest (image tags, Alembic revision, size, SHA-256); dumps are owner-only (`0600`) and never overwritten, and are deleted only when `retention.keepLast` is set. Verified backups are listed in `status.lifecycle.backups`, and failures show the failing step and error output in `status.lifecycle.backup.message` (for example a `pg_dump` older than the server, with the image tag to use). It runs at most once per lifecycle run and is not part of the task checksum cascade, so enabling or editing it never re-runs another task, and a retried migrate reuses the snapshot taken before the first attempt. It is mutually exclusive with `seed` and requires no additional RBAC. Status gains `status.lifecycle.backup`, `status.lifecycle.backups`, `status.lifecycle.settledChecksum`, and the `BackingUp` lifecycle phase ([@younsl](https://github.com/younsl)).
