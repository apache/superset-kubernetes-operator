...
- Every operator-created container (component Deployments, lifecycle task Jobs, and the create-database init container) now defaults to a hardened securityContext — `allowPrivilegeEscalation: false`, all capabilities dropped, and `seccompProfile: RuntimeDefault` — filled only where you have not set them yourself, so workloads satisfy the baseline Pod Security Standard out of the box. `runAsNonRoot`/`runAsUser` remain a user opt-in (the Superset image declares a named user, so a numeric UID cannot be safely defaulted across vanilla and OpenShift clusters). This changes pod templates, so workloads roll once on upgrade ([#363](https://github.com/apache/superset-kubernetes-operator/pull/363), [@villebro](https://github.com/villebro)).
