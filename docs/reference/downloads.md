...
!!! note
    The **signed source archive is the official Apache release**. The operator image and Helm chart are convenience binaries published to the GitHub Container Registry: `dev` / `sha-<commit>` tags and the `0.0.0-dev` chart on every merge to `main`, `<version>-rc<N>` tags for release candidates, and `<version>` + `latest` for final releases.

## Source Release

Per the [ASF Release Policy](https://www.apache.org/legal/release-policy.html), the **signed source archive is the official Apache release**; the operator image and Helm chart below are convenience binaries built from it.

Signed source archives, detached PGP signatures, and SHA-512 checksums are published to the ASF distribution site under `https://downloads.apache.org/superset/kubernetes-operator-<version>/`, for example `kubernetes-operator-0.1.0/`:
...
Multi-architecture operator images (`linux/amd64`, `linux/arm64`) are published to the GitHub Container Registry on every merge to `main` and on version tags:
...
- **Production**: Pin to a semver tag (e.g., `0.1.0`) or a `sha-` tag for full reproducibility.
- **Testing pre-release features**: Use an RC tag (e.g., `0.1.0-rc1`) or `dev`.
- **Avoid** using `latest` or `dev` in production — these are mutable and will change without notice.
...
Replace `<version>` with a chart version from the table below. For the latest development build, use `0.0.0-dev`.
...
For development or to test unreleased changes, install directly from a source checkout:
