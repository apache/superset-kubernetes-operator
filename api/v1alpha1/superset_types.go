...
// +kubebuilder:validation:XValidation:rule="!has(self.spec.serviceAccount) || (has(self.spec.serviceAccount.create) && self.spec.serviceAccount.create == false) || !has(self.spec.serviceAccount.name) || self.spec.serviceAccount.name == self.metadata.name",message="serviceAccount.name must equal metadata.name when the operator creates the ServiceAccount (serviceAccount.create is true or unset); set serviceAccount.create=false to reference a pre-existing ServiceAccount with a different name"
// +kubebuilder:validation:XValidation:rule="!((!has(oldSelf.spec.environment) || oldSelf.spec.environment == 'Production') && (has(self.spec.environment) && self.spec.environment != 'Production'))",message="environment cannot be changed from Production to Development or Staging in place (it would re-arm inline secrets and the destructive seed against production references); delete and recreate the Superset to change environment out of Production"
