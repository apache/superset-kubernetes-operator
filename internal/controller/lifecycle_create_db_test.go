...
// TestBuildCreateDatabaseInitContainer covers buildCreateDatabaseInitContainer:
// when it is skipped (disabled/defensive), the Postgres and MySQL scripts and
// images, port/credential env wiring (including passwordless and funky values),
// and inheritance of resources/securityContext from the migrate container template.
func TestBuildCreateDatabaseInitContainer(t *testing.T) {
	...
	t.Run("postgres", func(t *testing.T) {
		...
		if want := common.SeedImagePostgres; ctr.Image != want {
	...
	t.Run("mysql", func(t *testing.T) {
		...
		if want := common.SeedImageMySQL; ctr.Image != want {
			t.Errorf("expected image %q, got %q", want, ctr.Image)
		}
		script := ctr.Command[2]
		for _, want := range []string{
	...
	t.Run("funky credentials", func(t *testing.T) {
	...
	t.Run("defensive on missing fields", func(t *testing.T) {
	...
	t.Run("inherits from migrate container template", func(t *testing.T) {
...
// TestBuildStandardTaskFlatSpec covers buildStandardTaskFlatSpec end-to-end: that
// container hardening propagates into the create-database init container, that a
// user init container with the reserved name is dropped, and that the create-database
// init container is attached only to the migrate task and only when enabled.
func TestBuildStandardTaskFlatSpec(t *testing.T) {
	t.Run("inherits container hardening on migrate", func(t *testing.T) {
	...
	t.Run("drops user init container with reserved name", func(t *testing.T) {
		...
		if createDB.Image != common.SeedImagePostgres && createDB.Image != common.SeedImageMySQL {
	...
	t.Run("attaches create-db only to migrate", func(t *testing.T) {
		superset := &supersetv1alpha1.Superset{}
		superset.Name = "demo"
		superset.Spec.SecretKeyFrom = &corev1.SecretKeySelector{
			LocalObjectReference: corev1.LocalObjectReference{Name: "secret"},
	...
	t.Run("no create-db init when disabled", func(t *testing.T) {
		superset := &supersetv1alpha1.Superset{}
		superset.Name = "demo"
		superset.Spec.SecretKeyFrom = &corev1.SecretKeySelector{
			LocalObjectReference: corev1.LocalObjectReference{Name: "secret"},
